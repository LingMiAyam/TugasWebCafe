    ...
    <nav class="flex justify-center mt-6 space-x-2">
        ...
        @else
            <a href="{{ $paginator->previousPageUrl() }}"
               class="px-3 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition">←</a>
        @endif
        ...
        @foreach ($elements as $element)
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    ...
                    @else
                        <a href="{{ $url }}"
                           class="px-3 py-2 bg-stone-200 text-stone-700 rounded-lg hover:bg-stone-300 transition">{{ $page }}</a>
                    @endif
                @endforeach
            @endif
        @endforeach
